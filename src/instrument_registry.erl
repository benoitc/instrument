...
init([]) ->
  ...
  %% Own the exemplar reservoir table here so it lives for the registry's
  %% lifetime instead of being lazily created (and orphaned) by whatever
  %% transient process first records a histogram.
...
do_unreg_metric(Name) ->
  ...
  instrument_lib:safe_apply(persistent_term, erase, [{instrument_metric, Name}], ok),
  instrument_lib:safe_apply(persistent_term, erase, [{instrument_label_overflow, Name}], ok),
